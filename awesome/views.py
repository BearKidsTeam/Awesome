...
def thread_list(request):
	...
	page = [{'title': 'Awesome', 'url': reverse('home_page')}, {'title': 'Recommand List', 'url': reverse('thread_list')}]
	return render(request, 'awesome/thread_list.html', {'threads': threads, 'page':page})

def thread_detail(request, pk):
	thread = get_object_or_404(Post, pk=pk) 
	page = [{'title': 'Awesome', 'url': reverse('home_page')}, {'title': 'Recommand List', 'url': reverse('thread_list')}, {'title': 'Detail', 'url': '#'}]
